...
export default class ProjectControls extends Component{
  ...
  handleUnArchive(){
    ...
    Backend.updateProject(this.props.id, project).then(
    this.setState({
      dialogOpen : false,
      snackbar : true,
  ...
  loadComments(){
    alert(this.props.id)
    Backend.getProjectComments(this.props.id).then(function(data) {
  ...
  handleEdit(event){
    ...
    history.push("/update/" + this.props.id)
  }
  ...
  handleBookmark(event){
    event.preventDefault()
    this.handleClose()
    Backend.handleBookmark(this.props.id, this.props.projectInf.is_bookmark ).then(res => {
      var projectInf = this.props.projectInf
          projectInf.is_bookmark = this.props.projectInf.is_bookmark === "true" ? "false" : "true"
    })
